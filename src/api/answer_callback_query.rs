...
impl<'t> TGReq for AnswerCallbackQuery<'t> {
  type Resp = JsonTrueToUnitResp;

  fn request(&self) -> TGBotResult<HttpReq> {
    HttpReq::json_req(Method::POST, "answerCallbackQuery", self)
  }
}
...
impl<'t> AnswerCallbackQuery<'t> {
  ...
  pub fn url<T>(&mut self, url: T) -> &mut Self where T: Into<Cow<'t, str>> {
