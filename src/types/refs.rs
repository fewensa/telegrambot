...
impl ToChatRef for ForwardFrom {
  fn to_chat_ref(&self) -> ChatRef {
    match *self {
      ...
      ForwardFrom::ChannelHiddenUser { ref sender_name, .. } => ChatRef::HiddenUser(sender_name.clone()),
    }
  }
}
...
impl Serialize for ChatRef {
    ...
    where S: Serializer
  {
    match *self {
      ...
      ChatRef::HiddenUser(ref sender_name) => Err(serde::ser::Error::custom("The account was hidden by the user"))
    }
  }
}
